...
public class code05_迷宫 {
    ...
    public static String BFS(int[][] map, int row, int column) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                // exit
                if (x == row - 1 && y == column - 1 && map[x][y] == 0 && visit[x][y] == 0) {
                    // 打印路径
                    Node top = stack.pop();
                    sb.append(d);
                ...
                // bfs
                if (x >= 0 && x < row && y >= 0 && y < column && map[x][y] == 0 && visit[x][y] == 0) {
